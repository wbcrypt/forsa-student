...
const statusMap: Record<string, { color: string; label: string }> = {
  ...
  more_info_required: { color: 'bg-amber-50 text-amber-700', label: 'More Information Needed' },
  ...
  university_confirmed: { color: 'bg-cyan-50 text-cyan-700', label: 'University Confirmed' },
  university_paid: { color: 'bg-teal-50 text-teal-700', label: 'University Paid' },
...
export function Alert({ type = 'info', message, onClose }: {
  ...
  return (
    <div className={clsx('flex items-start gap-3 p-4 rounded-xl border text-sm mb-4', bg)}>
      ...
      {onClose && <button onClick={onClose} aria-label="Dismiss" className="opacity-60 hover:opacity-100"><X size={14} /></button>}
    </div>
  )
}
